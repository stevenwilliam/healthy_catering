...
func signer(now func() time.Time) *TokenSigner {
	return NewTokenSigner("current-signing-key-at-least-32-bytes-long", "", "evermore", 15*time.Minute, now)
}
...
func TestTokenIssueAndParse(t *testing.T) {
	...
	none := jwt.NewWithClaims(jwt.SigningMethodNone, Claims{
		RegisteredClaims: jwt.RegisteredClaims{Issuer: "evermore", Subject: subject.String(),
...
func TestTokenKeyRotation(t *testing.T) {
	...
	old := NewTokenSigner("old-signing-key-at-least-32-bytes-long!!", "", "evermore", time.Hour, clock)
	...
	rotated := NewTokenSigner("new-signing-key-at-least-32-bytes-long!!",
		"old-signing-key-at-least-32-bytes-long!!", "evermore", time.Hour, clock)
	...
	final := NewTokenSigner("new-signing-key-at-least-32-bytes-long!!", "", "evermore", time.Hour, clock)
